...
全局超时配置控制整个工作流的最大执行时间。如果工作流执行时间超过此限制，将被强制取消并标记为失败。**默认值为 30 分钟**。

```yaml
config:
  timeout: "30m"  # 如 "30m"（30分钟）、"1h"（1小时）
```

**使用场景**：
- 防止工作流因卡住或死循环而无限期占用资源
- 确保定时任务不会堆积（如每 5 分钟执行一次的任务如果执行超过 5 分钟，下次触发会被跳过）
- 为长时间运行的任务设置更长的超时（如数据处理任务设置为 "2h"）

**注意事项**：
- 超时时间格式为 Go duration，如 `"30m"`、`"1h"`、`"2h30m"`
- 未配置时使用默认值 30 分钟
- 超时后工作流状态会变为 `cancelled`，并触发完成通知（如果配置了通知频道）
...
#### Cron 触发器
- Service 启动后，以 60 秒为周期检查所有已启用工作流的 cron 表达式
- 使用 `gronx.NextTickAfter` 计算下次触发时间，并验证当前时间是否在匹配的分钟窗口内
- **精度**：仅在当前时间落在 cron 表达式指定的精确分钟内时触发（例如 `0 12 * * *` 在 12:00:00-12:00:59 之间触发）
- **不会提前触发**：不会在预定时间之前触发（例如 12:00 的任务不会在 11:59:31 触发）
- 同一工作流同时只允许一个实例运行（90 秒去重窗口）
- 支持时区设置（`tz` 字段）
- 去重机制：相同工作流 + 相同 cron 表达式在 90 秒内不会重复触发

#### At 触发器（一次性）
- 在指定的日期时间执行一次
- 时间格式：`2025-05-15 09:00:00` 或 `2025-05-15 09:00`
- **执行窗口**：如果当前时间在预定时间的 60 秒内则触发（例如 `12:00:00` 在 12:00:00-12:01:00 之间触发）
- 支持时区设置（`tz` 字段）
- 执行后自动标记为已完成，不会重复触发
- 去重机制：每个 at 时间点只执行一次
...
- **首次执行**：Service 启动时立即触发（如果没有之前的执行记录）
- **后续执行**：仅在距离上次执行的时间 >= 配置的间隔后触发
- 支持时区设置（`tz` 字段）
- 去重机制：跟踪每个工作流 + 间隔组合的最后执行时间
...
**触发器如何评估：**
- 工作流服务运行一个轮询循环，每 60 秒检查所有已启用的工作流
- 对于每种触发器类型，服务使用时间逻辑来判断是否应该执行：

| 触发器类型 | 检查逻辑 | 精度 |
|-----------|---------|------|
| **cron** | 使用 `gronx.NextTickAfter` 计算下次触发时间，然后验证当前时间是否在匹配的分钟窗口内 | ±0-59 秒（在 cron 分钟内） |
| **at** | 检查当前时间是否在预定时间的 60 秒内 | 预定时间后 ±0-60 秒 |
| **interval** | 检查距离上次执行的时间 >= 配置的间隔 | 精确间隔执行 |
...
使用此配置时：
- ✅ 会在 12:00:00、12:00:30、12:00:59 触发（12:00 分钟内的任何时间）
- ❌ 不会在 11:59:31 触发（早于预定时间）
- ❌ 不会在 12:01:00 触发（12:00 分钟已过）

**去重机制：**
- 每种触发器类型都有内置的去重机制以防止多次执行：
  - `cron`：相同工作流 + 相同 cron 表达式在 90 秒内不会重复触发
  - `at`：每个 at 时间点只执行一次
  - `interval`：跟踪每个工作流 + 间隔组合的最后执行时间
...
| 函数 | 语法 | 说明 | 示例输出 |
|------|------|---------|----------|
| `now` | 当前 UTC 时间 | `2026-05-13 08:30:00` |
| `now_tz` | `{{.fn.now_tz "Asia/Shanghai"}}` | 指定时区的当前时间 | `2026-05-13 16:30:00` |
| `date` | 当前 UTC 日期 | `2026-05-13` |
| `date_tz` | `{{.fn.date_tz "Asia/Shanghai"}}` | 指定时区的当前日期 | `2026-05-13` |
| `unix` | 当前 Unix 时间戳 | `1747170600` |
| `days_ago` | `{{.fn.days_ago 7}}` | N 天前的日期 | `2026-05-06` |
| `days_from_now` | `{{.fn.days_from_now 3}}` | N 天后的日期 | `2026-05-16` |
| `hours_ago` | `{{.fn.hours_ago 24}}` | N 小时前的时间 | `2026-05-12 08:30:00` |
| `hours_from_now` | `{{.fn.hours_from_now 2}}` | N 小时后的时间 | `2026-05-13 10:30:00` |
| `minutes_ago` | `{{.fn.minutes_ago 30}}` | N 分钟前的时间 | `2026-05-13 08:00:00` |
| `minutes_from_now` | `{{.fn.minutes_from_now 15}}` | N 分钟后的时间 | `2026-05-13 08:45:00` |
| `weeks_ago` | `{{.fn.weeks_ago 2}}` | N 周前的日期 | `2026-04-29` |
| `day_of_week` | 当前星期几（1=周一, 7=周日） | `1` |
| `format_time` | `{{.fn.format_time "2006年01月02日"}}` | 自定义时间格式 | `2026年05月13日` |

**其他函数：**

| 函数 | 语法 | 说明 | 示例输出 |
|------|------|---------|----------|
| `env` | `{{.fn.env "HOME"}}` | 获取环境变量值 | `/root` |
