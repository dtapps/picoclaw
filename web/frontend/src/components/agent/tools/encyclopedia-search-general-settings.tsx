...
interface EncyclopediaSearchGeneralSettingsProps {
  draft: EncyclopediaSearchConfigResponse
  onUpdateDraft: EncyclopediaSearchDraftUpdater
}

export function EncyclopediaSearchGeneralSettings({
  draft,
  onUpdateDraft,
}: EncyclopediaSearchGeneralSettingsProps) {
  const { t } = useTranslation()

  return (
    <div className="space-y-4">
      <h3 className="text-foreground/80 text-[13px] font-bold tracking-widest uppercase">
        {t("pages.agent.tools.encyclopedia_search.global_settings", "General")}
      </h3>

      <div className="bg-card border-border/40 divide-border/40 divide-y overflow-hidden rounded-2xl border shadow-sm">
        <SettingRow
          label={t(
            "pages.agent.tools.encyclopedia_search.provider",
            "Primary Provider",
          )}
          description={t(
            "pages.agent.tools.encyclopedia_search.provider_description",
        ...
        <SettingRow
          label={t(
            "pages.agent.tools.encyclopedia_search.proxy",
            "Proxy Configuration",
          )}
          description={t(
            "pages.agent.tools.encyclopedia_search.proxy_description",
            "Optional global HTTP/S proxy for underlying encyclopedia requests.",
          )}
        >
          <Input
        ...
        <SettingRow
          label={t(
            "pages.agent.tools.encyclopedia_search.prefer_native",
            "Prefer Native Search",
          )}
          description={t(
            "pages.agent.tools.encyclopedia_search.prefer_native_hint",
            "Bypass external providers if the agent inherently supports encyclopedia search tools.",
          )}
        >
          <Switch
