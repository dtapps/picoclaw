...
import type { EncyclopediaSearchProviderConfig } from "@/api/tools"
...
interface EncyclopediaSearchProviderSettingsProps {
...
export function EncyclopediaSearchProviderSettings({
  providerLabelMap,
  settings,
  expandedProvider,
  onToggleProviderExpand,
  onUpdateDraft,
}: EncyclopediaSearchProviderSettingsProps) {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-foreground/80 text-[13px] font-bold tracking-widest uppercase">
        {t(
          "pages.agent.tools.encyclopedia_search.providers_config",
          "Integrations",
        )}
      </h3>
...
function ProviderCard({
  ...
  settings: EncyclopediaSearchProviderConfig
  isExpanded: boolean
  ...
  const apiKeyPlaceholder = maskedSecretPlaceholder(
    ...
    t(
      "pages.agent.tools.encyclopedia_search.api_key_placeholder",
      "Enter API key...",
    ),
  )
  ...
  const updateSettings = (
    updater: (
      current: EncyclopediaSearchProviderConfig,
    ) => EncyclopediaSearchProviderConfig,
  ) => {
    ...
    >
      <div className="flex items-center justify-between gap-4 p-5">
        <button
          ...
          aria-controls={`encyclopedia-search-provider-${providerId}`}
      ...
      {isExpanded && (
        <div
          id={`encyclopedia-search-provider-${providerId}`}
        ...
        >
          <div className="ml-8 flex max-w-xl flex-col gap-5">
            <ProviderField
              label={t(
                "pages.agent.tools.encyclopedia_search.max_results",
                "Max Results",
              )}
            >
              <Input
            ...
            {baseUrlProviders.has(providerId) && (
              <ProviderField
                label={t(
                  "pages.agent.tools.encyclopedia_search.base_url",
                  "Base URL",
                )}
              >
                <Input
                  ...
                  placeholder={t(
                    "pages.agent.tools.encyclopedia_search.base_url_placeholder",
            ...
            {apiKeyProviders.has(providerId) && (
              <ProviderField
                label={t(
                  "pages.agent.tools.encyclopedia_search.api_key",
                  "API Key / Token",
                )}
                className="pt-1"
              >
