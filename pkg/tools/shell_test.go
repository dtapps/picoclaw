...
// TestShellTool_EnvVarsWithEnvFile verifies that env file variables are loaded and config overrides them
func TestShellTool_EnvVarsWithEnvFile(t *testing.T) {
	// Create a temporary .env file
	tmpDir := t.TempDir()
	envFile := filepath.Join(tmpDir, "test.env")

	envContent := `FILE_VAR=file_value
SHARED_VAR=from_file`
	...
	args := map[string]any{
		"action":  "run",
		"command": "echo $FILE_VAR $CONFIG_VAR $SHARED_VAR",
	}

	result := tool.Execute(ctx, args)

	require.False(t, result.IsError, "command should succeed: %s", result.ForLLM)
	require.Contains(t, result.ForLLM, "file_value", "FILE_VAR from env file should be present")
	require.Contains(t, result.ForLLM, "config_value", "CONFIG_VAR from config should be present")
	require.Contains(t, result.ForLLM, "from_config", "SHARED_VAR should be overridden by config")
	require.NotContains(t, result.ForLLM, "from_file", "SHARED_VAR should not have file value")
}
