...
- tmux 支持 macOS/Linux。在 Windows 上，使用 WSL 并在 WSL 内部安装 tmux。
- 此技能仅限于 `darwin`/`linux`，并且需要在 PATH 上有 `tmux`。
...
**提示：**
- 使用单独的 git worktrees 进行并行修复（无分支冲突）
- 在 fresh clones 中运行 codex 之前先执行 `pnpm install`
...
`{baseDir}/scripts/wait-for-text.sh` 用超时轮询窗格中的正则表达式（或固定字符串）。
