---
name: summarize
description: 从 URL、播客和本地文件中总结或提取文本/转录内容（非常适合作为"转录这个 YouTube/视频"的备用方案）。
...
- "transcribe this YouTube/video"（尽力提取转录内容；无需 `yt-dlp`）
...
如果用户要求转录但内容很大，先返回一个精简的总结，然后询问要展开哪个部分/时间范围。
...
- `--youtube auto`（如果设置了 `APIFY_API_TOKEN`，使用 Apify 备用）
